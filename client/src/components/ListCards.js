...
export default function ListCards(props) {
    ...
    if (loading) {
        return <Loader className="loader" type="ThreeDots" color="#FFFFFF" height={15} width={60} />;
    }  

    if (!cards.length) {
        return <EmptyList>
            <List>
        <li>{`Empty list of credit cards ! `} <a href="javascript:void(0)" onClick={handleInputFocus}>Add New Card</a></li></List></EmptyList>
    }
    ...
    return (
        <React.Fragment>
            <List>
                ...
                {cards.map(c => <li key={c._id}>{renderRowData(c)}</li>)}
            </List>
        </React.Fragment>
    );
}
